...
import { optionaleDatei } from './optionaleDatei.ts'
...
/**
 * Der Loader für eine Datei, die es in einer Klasse geben KANN.
 *
 * Steht seit dem Stundenplan in `optionaleDatei.ts` — zwei Sammlungen benutzen
 * ihn. Hier bleibt der Name als Re-Export stehen, damit die Aufrufstellen
 * (`astro/content.config.ts`, die Tests) unverändert bleiben und der Umzug
 * nicht in einem Aufwasch mit dem Stundenplan überprüft werden muss.
 */
export { optionaleDatei }
